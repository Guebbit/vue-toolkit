...
describe('useStructureFormManagement', () => {
  ...
  describe('validate (with schema)', () => {
    ...
    it('surfaces the correct zod error messages', () => {
      ...
      expect(composable.formErrors.value.password).toContain('Password must be at least 8 characters');
    });
  ...
  describe('handleSubmit', () => {
    ...
    it('calls the handler with form data when validation passes', async () => {
      ...
      expect(handler).toHaveBeenCalledWith({ email: 'valid@test.com', password: 'validPassword' });
    });
